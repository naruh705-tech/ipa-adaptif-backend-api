...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    query = query.order("nomor", { ascending: true }).range(offset, offset + limit - 1);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const maxNomor = (maxData && maxData.length > 0 && (maxData[0] as { nomor: number }).nomor) || 0;
    const nomor = maxNomor + 1;
