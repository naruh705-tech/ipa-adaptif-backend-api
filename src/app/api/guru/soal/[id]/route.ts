...
export async function DELETE(
  ...
  try {
    ...
    const { data: existingData, error: selectError } = await getSupabase()
      .from("soal")
      .select("nomor")
      .eq("id", id)
      .single();
    ...
    const deletedNomor = existingData.nomor;

    const { error: deleteError } = await getSupabase().from("soal").delete().eq("id", id);
    if (deleteError) {
    ...
    if (laterError) {
      return errorResponse(laterError.message, 500);
    }
