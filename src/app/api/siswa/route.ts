...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = getSupabase()
      .from("siswa")
      .select("id, nisn, nama, kelas, status, foto_profil, created_at, updated_at", { count: "exact" });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { nisn, nama, kelas, password, status } = body;

    if (!nisn || !nama || !kelas || !password) {
      return errorResponse("NISN, nama, kelas, dan password harus diisi", 400);
    }
    ...
    const { data, error } = await getSupabase()
      ...
      .select("id, nisn, nama, kelas, status, foto_profil, created_at")
      .single();
